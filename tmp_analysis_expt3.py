...
for i in range(4, 6):
    ...
    con_true = [control[ii].mean() for ii in range(1, 49)]
    sr_true = [sr[i].mean() for i in range(1, 49)]
    # compare each frequency between patients and controls
    eff_size_true = [cohen_d(sr[ii], control[ii]) for ii in range(1, 49)]
    pval_true = [pg.ttest(control[ii], sr[ii])['p-val'][0] for ii in range(1, 49)]
    repetitions = 1000
...
f_titles = ['Experiment 3 - 10050 vs 20100', 'Experiment 3 - 10100 vs 20100']
...
for i in range(4, 6):
    ...
    fig.suptitle(f_titles[ii], fontsize=10)
    ...
    main_subplot.scatter(freq_vector - 0.1 + 1, con_dprimes[ii], s=30, facecolors='none', edgecolors='blue')
    main_subplot.errorbar(freq_vector - 0.1 + 1, con_dprimes[ii], yerr=ci_dprimes[ii], capsize=2, color='blue', elinewidth=0.5, linewidth=0.7)
    main_subplot.scatter(freq_vector + 0.1 + 1, sr_dprimes[ii], s=30, facecolors='none', edgecolors='red')
    main_subplot.errorbar(freq_vector + 0.1 + 1, sr_dprimes[ii], yerr=ci_dprimes[ii], capsize=2, color='red', elinewidth=0.5, linewidth=0.7)
    main_subplot.fill_between([best_freq[ii] - highlight_size/2, best_freq[ii] + highlight_size/2], -0.4, 2.3, color=highlight_color, alpha=highlight_alpha)
    plt.text(best_freq[ii], 2.1, "Cohen's d = " + str(best_fx[ii]), horizontalalignment='left', verticalalignment='center', bbox=bbox_props)
    ...
    sub1.set_title("SRs (" + str(best_freq[ii]) + 'Hz)', fontsize=10)
    ...
    for ii_ in range(2):
        for jj in range(2):
            sub1.text(jj, ii_, f'{data[ii_, jj]:.2f}', ha='center', va='center', color='black', fontsize=8)

    # Subplot 2 in the second subplot
    sub2 = plt.Subplot(fig, inner_grid[1])
    sub2.set_title("Controls (" + str(best_freq[ii]) + 'Hz)', fontsize=10)
    ...
    for ii_ in range(2):
        for jj in range(2):
            sub2.text(jj, ii_, f'{data[ii_, jj]:.2f}', ha='center', va='center', color='black', fontsize=8)
...
for id_ in range(9, 14):
    for j in range(4, 6):
        ...
        prc_[0] = prc_[1] - prc_[0]
        prc_[2] = prc_[2] - prc_[1]
        plt.scatter(freq_vector + 1, prc_[1], s=5, c=pl_colors[j - 4])
        plt.errorbar(freq_vector + 1, prc_[1], yerr=prc_[[0, 2]], capsize=2, color=pl_colors[j - 4], elinewidth=0.5, linewidth=0.7)
        # perm data
        perm_ = ['k', 'gray']
        if plot_perm == 1 and j == 4:
            prc_ = np.percentile(rlp_, (25, 50, 75), axis=0)
            prc_[0] = prc_[1] - prc_[0]
            prc_[2] = prc_[2] - prc_[1]
            plt.scatter(freq_vector + 1, prc_[1], s=5, c=perm_[j - 4])
            plt.errorbar(freq_vector + 1, prc_[1], yerr=prc_[[0, 2]], capsize=2, color=perm_[j - 4], elinewidth=0.5, linewidth=0.7)
    ...
    plt.legend(['10050 vs 20100', 'random labels', '10100 vs 20100'], loc="upper right", ncol=1, fontsize=7)
