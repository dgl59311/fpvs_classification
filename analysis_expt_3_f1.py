...
acc_results = list(filter(lambda s: '_acc_expt_3.npy' in s, os.listdir(results_dir)))
f1_results = list(filter(lambda s: '_f1_expt_3.npy' in s, os.listdir(results_dir)))
sdt = list(filter(lambda s: 'expt_3_tp_fp_fn_tn' in s, os.listdir(results_dir)))
...
data_cat.to_csv(os.path.join(main_dir, 'results', 'sensitivity', 'Experiment_3_accuracy.csv'))
...
conditions = ['1050', '1020', '10100']
# results from experiment
results = list(filter(lambda s: '_acc_expt_3.npy' in s, os.listdir(results_dir)))
sdt = list(filter(lambda s: '_conf_mat_expt_3' in s, os.listdir(results_dir)))

# either range(9) for Experiment 3 - 1; or range(9, 14) for Experiment 3 - 2
...
for id_ in range(9, 14):
    ...
    to_plot = subj_confmat[:, :, :, ix].mean(0).reshape(3, 3)
    ix = ix + 1
    ...
    plt.ylim([0.3, 0.9])
    plt.yticks([0.3, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9], [0.3, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9])
    plt.xlim(-2, 52)
    if 'Control' in group_:
        g_id = 'control'
    else:
