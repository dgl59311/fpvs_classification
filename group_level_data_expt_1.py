...
# Experiment
exp_acc = pd.read_csv(os.path.join(results_dir, 'Experiment_1_accuracy.csv'), index_col=0)
...
bfs = [pg.ttest(alldata.values[:, i], 0.25,
...
plt.plot([-1, 100], [0.25, 0.25], linestyle='--', color='green', alpha=0.9)
...
for i in range(len(freq_vector)):
    ...
    rect = mpatches.Rectangle((freq_vector[i] - 0.3, 0.26), 0.6, 1, color=color, alpha=1, fill='none')
    plt.gca().add_patch(rect)
    color = category_colors[int_bfs[i]]
    rect = mpatches.Rectangle((freq_vector[i] - 0.3, 0), 0.6, 0.24, color=color, alpha=1, fill='none')
    plt.gca().add_patch(rect)

plt.xlabel('frequency')
plt.ylabel("accuracy")
plt.xlim(0.5, 47.5)
plt.ylim(0.18, 0.68)  # Adjust according to your data range
plt.yticks([0.3, 0.5, 0.7], [0.3, 0.5, 0.7])
...
plt.show()
plt.tight_layout()
plt.savefig(os.path.join(main_dir, 'results', 'figures', 'Group_Experiment_1.jpg'), dpi=500)
...
results = list(filter(lambda s: '_acc_expt_1.npy' in s, os.listdir(results_dir)))
idx_sr = [5, 9, 10]
idx_con = [4, 3, 6]
ixp = 0
delticks = 1
# change idx_sr to idx_con. SRs are 1_2
# change color 'red' to 'blue' for controls
for id_ in idx_con:

    subj = results[id_]
    subj_id = subj.replace('_acc_expt_1.npy', '')
    ...
    plt.scatter(freq_vector, prc_[1], s=5, c='blue', alpha=0.7)
    plt.errorbar(freq_vector, prc_[1], yerr=prc_[[0, 2]], capsize=1, color='blue', elinewidth=0.5, linewidth=0.6, alpha=0.5)
    plt.xlim(0.5, 47.5)
    ...
    if delticks == 1:
        ...
        plt.yticks([])
    plt.ylim(0.18, 0.68)  # Adjust according to your data range
    plt.plot([-1, 100], [0.25, 0.25], linestyle='--', color='green', alpha=0.4)
...
plt.tight_layout()
plt.savefig(os.path.join(main_dir, 'results', 'figures', 'Group_Experiment_1_3_dticks.jpg'), dpi=500)

# Create final figure by stacking the generated subplots
image1 = Image.open(os.path.join(main_dir, 'results', 'figures', 'Group_Experiment_1_2_dticks.jpg'))
image2 = Image.open(os.path.join(main_dir, 'results', 'figures', 'Group_Experiment_1.jpg'))
image3 = Image.open(os.path.join(main_dir, 'results', 'figures', 'Group_Experiment_1_3_dticks.jpg'))
...
combined_image.save(os.path.join(main_dir, 'results', 'figures', 'Experiment_1.jpg'))
combined_image.show()
