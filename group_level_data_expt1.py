...
plt.errorbar(freq_vector - 0.2, controldata.values.mean(0), yerr=controldata.std(0) / np.sqrt(len(controldata)),
...
plt.errorbar(freq_vector + 0.2, srdata.values.mean(0), yerr=srdata.std(0) / np.sqrt(len(srdata)),
...
plt.show()
plt.tight_layout()
plt.savefig(os.path.join(main_dir, 'results', 'figures', 'Group_Experiment_1_v2.pdf'), dpi=500)
